...
from app.schemas.users import UserLoginSchema, UserBase
...
class TestUserLogin(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self.valid_login = UserLoginSchema(
            fullname=self.config["TEST_USER"]["FULLNAME"],
            password=self.config["TEST_USER"]["PASSWORD"],
        )
        self.invalid_name = UserLoginSchema(
            fullname=self.config["TEST_USER"]["FULLNAME"] + "_wrong",
            password=self.config["TEST_USER"]["PASSWORD"],
        )
        self.invalid_password = UserLoginSchema(
            fullname=self.config["TEST_USER"]["FULLNAME"],
            password=self.config["TEST_USER"]["PASSWORD"] + "_wrong",
        )
