import unittest
from app.api import add_post, user_login, create_user, delete_user, logout, delete_posts, session, Users, Session
...
class TestAddPost(unittest.IsolatedAsyncioTestCase):
    ...
    valid_post = PostBase(title=config['TEST_POST']['TITLE'], text=config['TEST_POST']['TEXT'], create_date=config['TEST_POST']['CREATE_DATE'])
    valid_token = ''
    ...
    def test_add_post(self):
        ...
        result = user_login(UserLoginSchema(fullname = self.config['TEST_USER']['FULLNAME'],password = self.config['TEST_USER']['PASSWORD']))
    ...
    def tearDown(self):
        user_data = session.query(Users).filter_by(fullname=self.config['TEST_USER']['FULLNAME']).first()
        if user_data:
